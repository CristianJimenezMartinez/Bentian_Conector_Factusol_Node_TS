...
const ensureCAWCLIFieldLength_1 = require("./ensureCAWCLIFieldLength");
...
function crearBaseDeDatos() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const client = new pg_1.Client({
            ...
            port: config_json_1.default.database.port || 5432,
            database: 'postgres'
        });
        try {
            ...
            const res = yield client.query("SELECT 1 FROM pg_database WHERE datname = 'Factusol'");
            if (res.rowCount === 0) {
                yield client.query('CREATE DATABASE "Factusol"');
            ...
            const resUser = yield client.query("SELECT 1 FROM pg_roles WHERE rolname = 'user_factusol'");
            if (resUser.rowCount === 0) {
                yield client.query("CREATE USER user_factusol WITH PASSWORD '123456789'");
            ...
            yield client.query('GRANT ALL PRIVILEGES ON DATABASE "Factusol" TO user_factusol');
            yield client.query('GRANT CREATE, USAGE ON SCHEMA public TO user_factusol');
...
function conectar() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield crearBaseDeDatos();
            // Luego, se crea un nuevo cliente para la base de datos "Factusol"
            const clientFactusol = new pg_1.Client({
                host: config_json_1.default.database.host,
                user: config_json_1.default.database.user, // usuario administrador (o el que tenga permisos)
                password: config_json_1.default.database.password,
                database: config_json_1.default.database.database, // "Factusol"
                port: config_json_1.default.database.port || 5432
            });
            yield clientFactusol.connect();
            console.log('Conexión exitosa con la base de datos Factusol');
            // Crear (o verificar) la tabla de configuración
            yield crearTablaConfiguracion(clientFactusol);
            // Verificamos la existencia de una tabla clave (por ejemplo, f_cfg)
            const existe = yield verificarTablaExistente(clientFactusol, 'f_cfg');
            if (!existe) {
                yield crearTablaCfg(clientFactusol);
                yield crearTablaCli(clientFactusol);
                yield crearTablaAge(clientFactusol);
                yield crearTablaSec(clientFactusol);
                yield crearTablaFam(clientFactusol);
                yield crearTablaArt(clientFactusol);
                yield crearTablaTar(clientFactusol);
                yield crearTablaLta(clientFactusol);
                yield crearTablaDes(clientFactusol);
                yield crearTablaFac(clientFactusol);
                yield crearTablaLfa(clientFactusol);
                yield crearTablaFpa(clientFactusol);
                yield crearTablaAlm(clientFactusol);
                yield crearTablaPcl(clientFactusol);
                yield crearTablaLpc(clientFactusol);
                yield crearTablaSto(clientFactusol);
                yield crearTablaEmp(clientFactusol);
                yield crearTablaUsuarios(clientFactusol);
                yield (0, ensureCAWCLIFieldLength_1.ensureCAWCLIFieldLength)();
                console.log('Todas las tablas fueron creadas exitosamente');
            }
            else {
                console.log('Las tablas ya existen');
            }
            yield clientFactusol.end();
...
function verificarTablaExistente(client, tableName) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const resultado = yield client.query(`SELECT EXISTS (
...
function crearTablaUsuarios(client) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield client.query(`
...
function crearTablaConfiguracion(client) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            yield client.query(`
    ...
    `);
            ...
            const res = yield client.query("SELECT * FROM configuracion WHERE clave = 'inicializacion'");
            if (res.rowCount === 0) {
                yield client.query("INSERT INTO configuracion (clave, valor) VALUES ('inicializacion', 0)");
